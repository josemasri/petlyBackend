 ...
 * cuentas y delegar las peticiones al gestor 
...
routerCuentas.get("/renuevatoken", verificaToken, (req: any, res: Response) => {
  GestorCuentas.renuevaToken(
    req.usuario,
    (err: any, message: string, token: any) => {
      if (err) {
        res.status(500).json({
          ok: false,
          err,
...
routerCuentas.post("/registro", (req: Request, res: Response) => {
  ...
  GestorCuentas.agregarUsuario(
    ...
    (err: any, message: string, results: Object[]) => {
      if (err) {
...
routerCuentas.post("/login", (req: Request, res: Response) => {
  ...
  GestorCuentas.loginUsuario(
    body.email,
    body.password,
    (err: any, message: string, usuario: any, token: any, menu: any) => {
      if (err) {
...
routerCuentas.get(
  ...
  [verificaToken, verificaAdminRole],
  (req: Request, res: Response) => {
    GestorCuentas.obtenerVeterinarios(
      (err: any, message: string, results: Object[]) => {
        if (err) {
          return res.status(403).json({
            ok: false,
